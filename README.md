...
match the BOLD filename, and the behavioral file must be in the matching
`sub-*/ses-*/beh/` directory. Rest runs do not require behavioral input.
...
`exceptions`, and `errors`; use `--json PATH` to save the same payload.
...
`audit` exits 0 only when the behavioral directory exists and every non-rest BOLD
...
1. Onsets are shifted by the discarded-volume count times `RepetitionTime` from
   ...
   has a zero shift, so the correction cannot be applied twice.
